...
public sealed partial class DataTableProcessor
{
    ...
    public DataTableProcessor(string dataTableFileName, Encoding encoding, int contentStartRow, int nameRow)
    {
        ...
        {
            throw new Exception(Utility.TextUtility.Format("Data table file '{0}' is not a txt.", dataTableFileName));
        }

        if (!File.Exists(dataTableFileName))
        {
            throw new Exception(Utility.TextUtility.Format("Data table file '{0}' is not exist.", dataTableFileName));
        }
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            {
                throw new Exception(Utility.TextUtility.Format(
                    "Data table file '{0}', raw Column is '{2}', but line '{1}' column is '{3}'.", dataTableFileName, i,
                    rawColumnCount,
                    rawValue.Length));
            }
        ...
        {
            throw new Exception(Utility.TextUtility.Format("Content start row '{0}' is invalid.", contentStartRow));
        }
        ...
        int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
    {
        ...
        if (nameRow < 0)
        {
            throw new Exception(Utility.TextUtility.Format("Name row '{0}' is invalid.", nameRow));
        }

        if (typeRow < 0)
        {
            throw new Exception(Utility.TextUtility.Format("Type row '{0}' is invalid.", typeRow));
        }
        ...
        if (idColumn < 0)
        {
            throw new Exception(Utility.TextUtility.Format("Id column '{0}' is invalid.", idColumn));
        }

        if (nameRow >= rawRowCount)
        {
            throw new Exception(Utility.TextUtility.Format("Name row '{0}' >= raw row count '{1}' is not allow.",
                nameRow, rawRowCount));
        }

        if (typeRow >= rawRowCount)
        {
            throw new Exception(Utility.TextUtility.Format("Type row '{0}' >= raw row count '{1}' is not allow.",
                typeRow, rawRowCount));
        }
        ...
        {
            throw new Exception(Utility.TextUtility.Format(
                "Default value row '{0}' >= raw row count '{1}' is not allow.", defaultValueRow.Value, rawRowCount));
        }

        if (commentRow.HasValue && commentRow.Value >= rawRowCount)
        {
            throw new Exception(Utility.TextUtility.Format("Comment row '{0}' >= raw row count '{1}' is not allow.",
                commentRow.Value, rawRowCount));
        }
        ...
        {
            throw new Exception(Utility.TextUtility.Format("Id column '{0}' >= raw column count '{1}' is not allow.",
                idColumn, rawColumnCount));
        }
    ...
    public bool IsIdColumn(int rawColumn)
    {
        ...
        {
            throw new Exception(Utility.TextUtility.Format("Raw column '{0}' is out of range.", rawColumn));
        }
    ...
    public bool IsCommentRow(int rawRow)
    {
        ...
        {
            throw new Exception(Utility.TextUtility.Format("Raw row '{0}' is out of range.", rawRow));
        }
    ...
    public string GetString(int index)
    {
        ...
        {
            throw new Exception(Utility.TextUtility.Format("String index '{0}' is out of range.", index));
        }
    ...
    public bool GenerateDataFile(string outputFileName)
    {
        ...
        {
            Debug.LogError(Utility.TextUtility.Format("Parse data table '{0}' failure, exception is '{1}'.",
    ...
    public bool SetCodeTemplate(string codeTemplateFileName, Encoding encoding)
    {
        ...
        {
            Debug.LogError(Utility.TextUtility.Format("Set code template '{0}' failure, exception is '{1}'.",
    ...
    public bool GenerateCodeFile(string outputFileName, Encoding encoding, object userData = null)
    {
        ...
        {
            Debug.LogError(Utility.TextUtility.Format("Generate code file '{0}' failure, exception is '{1}'.",
                outputFileName, exception));
            return false;
        }
    }
            ...
            using (BinaryWriter binaryWriter = new BinaryWriter(memoryStream, Encoding.UTF8))
            {
                for (int rawColumn = 0; rawColumn < RawColumnCount; rawColumn++)
                {
                    ...
                    catch
                    {
                        ...
                        {
                            Debug.LogError(Utility.TextUtility.Format(
                                "Parse raw value failure. OutputFileName='{0}' RawRow='{1}' RowColumn='{2}' Name='{3}' Type='{4}' RawValue='{5}'",
                                outputFileName,
                                rawRow, rawColumn, GetName(rawColumn), GetLanguageKeyword(rawColumn),
                        ...
                        else
                        {
                            Debug.LogWarning(Utility.TextUtility.Format(
                                ...
                                outputFileName, rawRow,
                                rawColumn, GetName(rawColumn), GetLanguageKeyword(rawColumn),
                                GetValue(rawRow, rawColumn)));
                            try
                            {
