...
{
    private const string DataTablePath = "Assets/FastDev/DataTables";
    private const string CSharpCodePath = "Assets/FastDev/Scripts/DataTable/Tables";
    private const string CSharpCodeTemplateFileName = "Assets/FastDev/Configs/DataTableCodeTemplate.txt";
    ...
    {
        return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(DataTablePath, datableName + ".txt")),
            Encoding.GetEncoding("GB2312"), 2, 3);
    }
    ...
    public static bool CheckRawData(DataTableProcessor dataTableProcessor, string dataTableName)
    {
        for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
        {
            ...
            {
                Debug.LogWarning(Utility.TextUtility.Format("Check raw data failure. DataTableName='{0}' Name='{1}'",
                    dataTableName, name));
                return false;
            }
        }
    ...
    private sealed class PropertyCollection
    {
        ...
        public KeyValuePair<int, string> GetItem(int index)
        {
            ...
            {
                throw new Exception(Utility.TextUtility.Format("GetItem with invalid index '{0}'.",
                    index));
            }
